...
class TuitionController {
    ...
    getById = async (req, res) => {
        ...
        Tuition.findOne({ _id: req.params.id })
    ...
    getItemById = async (req, res) => {
        ...
        if (!!req.query.id) {
            Tuition.findOne({ _id: req.query.id })
    ...
    create = async (req, res) => {
        ...
        const newItem = new Tuition(req.body);
        newItem
            .save()
            .then(
                (result) =>
    ...
    update = async (req, res) => {
        if (!!req.query.id && !!Object.keys(req.body).length) {
            Tuition.updateOne({ _id: req.query.id }, req.body)
                .then((result) => {
                    res.status(200).json({
                        status: true,
    ...
    delete(req, res, next) {
        // soft delete
        if (!!req.body.id) {
            Tuition.delete({ _id: req.body.id })
    ...
    restore(req, res, next) {
        if (!!req.query.id) {
            Tuition.restore({ _id: req.query.id })
    ...
    destroy(req, res, next) {
        if (!!req.query.id) {
            delete Tuition.deleteOne({ _id: req.query.id })
